...
export async function exportMix(
  ...
  // O tempo de renderização no contexto offline depende da velocidade (playbackRate)
  const renderDuration = (safeEndTime - safeStartTime) / playbackRate;
  ...
  // 4. Configurar o filtro Bass Boost se estiver ativado
  let lastNode: AudioNode = offlineCtx.destination;
  if (isBassBoostEnabled) {
    const boostFilter = offlineCtx.createBiquadFilter();
    boostFilter.type = 'lowshelf';
    boostFilter.frequency.value = 100;
    boostFilter.gain.value = 12; // Valor fixo conforme definido no hook
    boostFilter.connect(offlineCtx.destination);
    lastNode = boostFilter;
  }
  ...
  buffersToMix.forEach(({ buffer, volume }) => {
    ...
    // Inicia no tempo 0 do contexto offline, lendo a partir do safeStartTime do buffer original
    // O parâmetro 'duration' no source.start também deve ser ajustado ou omitido para tocar até o fim do render
    source.start(0, safeStartTime, safeEndTime - safeStartTime);
  });
